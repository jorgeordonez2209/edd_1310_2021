...
# curr_node ->>>> todo el nodo #curr_node.data-->>>> dato que hay en el nodo
    ...
    def remove(self, value):
        curr_node=self.__head
        if self.__head.data==value: #para poder eliminar primer nodo
    ...
    def get(self, posicion=None): #por defecto regresa el ultimo
        ...
        if posicion==None:
            ...
            if self.__head.data==posicion:
                dato=self.__head.data
            else:
                contador=0
                while self.__head.data != posicion and self.__head.siguiente != None:
                    contador=self.__head
                    self.__head=self.__head.siguiente
                if self.__head.data==posicion:
                    dato=self.__head.data
                else:
                    print("No exite valor")
        return dato
